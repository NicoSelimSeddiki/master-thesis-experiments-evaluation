...
# Type annotations
from TypeAnnotations import (Dict, List, Tuple, Union, 
    DataFrame, DataSeries, ArrayLike)
...
class PlotHelper():
    ...
    _legend_title_fontsize: float = 16
    _legen_labels_fontsize: float = 14

    # General - location of title
    _x_title: float = 0.5
    _y_title: str = 0.965
    
    _x_label_fontsize: float = 15
    _y_label_fontsize: float = 15
    ...
    _x_ticks: List[str] = [
    '10_20','20_30','30_40','40_50','50_60','60_70','70_80','80_90']
    

    # Static methods:----------------------------------------------------------
    @staticmethod     
    def clusters(cluster1: DataSeries, cluster2: DataSeries, 
        feat1: str, feat2: str) -> None:
    ...
    @staticmethod    
    def scatterplot(feature1: DataSeries, feature2: DataSeries, 
        xl: str, yl: str) -> None:
        """ Simple Scatterplot
    ...
    def hgram(data: List[DataFrame], feature: str) -> None:
        ...
        # Set figure name
        fig.suptitle(f"Histogram for {feature}",
            x=PlotHelper._x_title, 
            y=PlotHelper._y_title
        )
    ...
    @staticmethod
    def lineplot(*args, name: str, description: List[str], 
    xl:str, yl:str) -> None:
        ...
        plt.gca().set_prop_cycle(
            marker=PlotHelper._markers, 
            color=PlotHelper._colors)
     
        for arg in args:
            ax.plot(PlotHelper._x_ticks, arg, 
    ...
    @staticmethod
    def qplot(data: Union[ArrayLike, DataFrame], 
        answers: List[DataFrame], xl: str) -> None:
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                ax[i, j].scatter(data, answers[j].iloc[i], 
                color=PlotHelper._colors[j], alpha=alphas, 
                s=m_size+2.0
            )
            
            # set x-axis labels and position
            ax[0, i].xaxis.set_label_position('top')
            ax[0, i].set_xlabel(PlotHelper._questions[i], 
                labelpad=PlotHelper._labelpad // 2
            )
            
            # set y-axis labels
            ax[i, 0].set_ylabel(PlotHelper._answers[i], 
        ...
        text_Q1 = TextArea("Q1: Voice feel during the last 10 min.?",
            textprops=text_prop)
        ...
        text: List[TextArea] = list(
            map(lambda txt: TextArea(txt, textprops=text_prop), [
                "Q1: Voice feel during the last 10 min.?", 
                "Q2: Voice feel right now?", 
                "Q3: Feeling in general?"])
            )

        box_Q: VPacker = VPacker(children=[text_Q, text_Q1, text_Q2, text_Q3], 
            align="left", pad=0, sep=20)
        ...
        text_A1 = TextArea("A: not strained(0) - strained(10)",
            textprops=text_prop)

        text_A2 = TextArea("B: rested(0) - not rested(10)", 
            textprops=text_prop)

        text_A3 = TextArea("C: smooth(0) - rough(10)", 
            textprops=text_prop)

        box_A: VPacker = VPacker(children=[text_A, text_A1, text_A2, text_A3], 
            align="left", pad=0, sep=20)
        ...
        # Set figure name
        fig.suptitle(f"Relationship between SRA and {xl}", 
            x=PlotHelper._x_title, 
            y=PlotHelper._y_title
        )
    ...
    def _set_patches(cls, len_data: int = 1) -> List[mpatches.Circle]:
        ...
        return [mpatches.Circle(xy=cls._xy, radius=cls._radius, 
            facecolor=cls._colors[i]) for i in range(len_data)]